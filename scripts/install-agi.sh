...
DIALPLAN_FILE="/tmp/vicidial-did-optimizer-dialplan.conf"
...
generate_dialplan() {
    print_info "Generating dialplan configuration..."

    cat > "$DIALPLAN_FILE" <<'EOF'
; =============================================================================
; VICIdial DID Optimizer - Dialplan Configuration
; Generated by install-agi.sh
; =============================================================================

; Add these two lines to your VICIdial Admin Carrier Dialplan Entry:
; (Admin → Carriers → Select your carrier → Dialplan Entry section)
...
; 3. Click on your outbound carrier
; 4. Scroll to "Dialplan Entry" section
; 5. ADD THE ABOVE TWO LINES **BEFORE** your Dial() command
; 6. Click "Submit" to save
;
; NOTE: Adjust the pattern (_91NXXNXXXXXX) to match your dialing pattern:
;   - _91NXXNXXXXXX  = US/Canada (1 + area code + number)
;   - _9NXXXXXXXXX   = 10-digit dialing
;   - _9011.         = International
;   - _9NXX          = For testing with 3-digit extensions
;
; IMPORTANT: Do NOT edit /etc/asterisk/extensions.conf directly!
; VICIdial manages the dialplan through its admin interface.
; =============================================================================
EOF

    chmod 644 "$DIALPLAN_FILE"
    print_step "Dialplan configuration generated: $DIALPLAN_FILE"
}
...
print_next_steps() {
    ...
    echo -e "   ${GREEN}✓ Dialplan configuration generated: ${DIALPLAN_FILE}${NC}\n"
    echo -e "   ${RED}⚠️  DO NOT edit /etc/asterisk/extensions.conf directly!${NC}"
    echo -e "   Instead, use VICIdial Admin interface:\n"
    echo -e "   - Open the generated file to copy the dialplan:"
    echo -e "     ${BLUE}cat $DIALPLAN_FILE${NC}\n"
    echo -e "   - Go to: ${BLUE}Admin → Carriers${NC}"
    echo -e "   - Click on your outbound carrier"
    echo -e "   - Scroll to ${BLUE}\"Dialplan Entry\"${NC} section"
    echo -e "   - Copy the two dialplan lines from the file"
    echo -e "   - Paste them ${YELLOW}BEFORE${NC} your Dial() command"
