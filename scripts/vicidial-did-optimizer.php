<?php
/**
 * VICIdial DID Optimizer Integration Script (PHP Version)
 *
 ...
 * 2. Configure API settings below
 * 3. Test with: php vicidial-did-optimizer.php test
 * 4. Or test via web: http://yourserver/vicidial-did-optimizer.php?test=1
 *
 * Usage:
 * Command line: php vicidial-did-optimizer.php campaign_id agent_id phone_number state zip
 * Web API: http://yourserver/vicidial-did-optimizer.php?campaign_id=X&agent_id=Y&phone_number=Z
 */

// Configuration
define('API_BASE_URL', 'http://localhost:3001');
define('API_KEY', 'did_250f218b1404c84b5eda3dbf87f6cc70e63ce2904d8efdd607aab2f3b7733e5a');
define('API_TIMEOUT', 10);
define('FALLBACK_DID', '+18005551234');
define('LOG_FILE', '/var/log/did_optimizer.log');
define('DEBUG', true);
...
class DIDOptimizer {
    ...
    private $api_key;
    private $timeout;
    private $fallback_did;

    public function __construct() {
        $this->api_base_url = API_BASE_URL;
        $this->api_key = API_KEY;
        $this->timeout = API_TIMEOUT;
        $this->fallback_did = FALLBACK_DID;
    }
    ...
    public function getOptimalDID($campaign_id, $agent_id, $phone_number = '', $state = '', $zip = '') {
        ...
        // Make API request
        $url = $this->api_base_url . '/api/v1/vicidial/next-did?' . http_build_query($params);
        ...
        if ($response && $response['success']) {
            ...
            // Store call context for result reporting
        ...
        return ['phoneNumber' => $this->fallback_did, 'algorithm' => 'fallback'];
    }
    ...
    public function reportCallResult($phone_number, $campaign_id, $result, $duration = 0, $disposition = '') {
        ...
        // Get customer data for AI training
        $customer_data = $this->getCustomerDataFromDB($phone_number, $campaign_id);

        $payload = [
            'phoneNumber' => $context['phone_number'] ?? $this->fallback_did,
        ...
        $url = $this->api_base_url . '/api/v1/vicidial/call-result';
    ...
    public function checkHealth() {
        $url = $this->api_base_url . '/api/v1/vicidial/health';
        $response = $this->makeAPIRequest('GET', $url);
    ...
    private function getCustomerLocation($phone_number, $state = '', $zip = '') {
        ...
        if (preg_match('/(\d{3})/', $phone_number, $matches)) {
        ...
        // If no state, try to get from area code
        if (empty($location['state']) && !empty($location['area_code'])) {
            $location['state'] = $this->areaCodeToState($location['area_code']);
        }

        // If no coordinates and we have state, use state center
        if (empty($location['latitude']) && !empty($location['state'])) {
    ...
    private function storeCallContext($did_data, $phone_number, $location_data) {
        ...
        $context_file = "/tmp/did_context_{$did_data['campaign_id']}_{$phone_number}.json";
    ...
    private function loadCallContext($campaign_id, $phone_number) {
        $context_file = "/tmp/did_context_{$campaign_id}_{$phone_number}.json";
    ...
    public function runTests() {
        echo "🧪 Running DID Optimizer Integration Tests...\n\n";
        ...
        $result = $this->reportCallResult('4155551234', 'TEST_CAMPAIGN', 'answered', 120, 'SALE');
        echo $result ? "   ✅ Call Result Reporting: PASSED\n" : "   ❌ Call Result Reporting: FAILED\n";

        echo "\n🎉 All tests completed!\n";
        echo "\n📋 Integration Instructions:\n";
        echo "1. Update API_BASE_URL and API_KEY constants in this script\n";
        echo "2. Install PHP with cURL and PDO extensions\n";
        echo "3. Place script in your web server directory\n";
        echo "4. Configure your VICIdial dialplan to call this script\n";
        echo "5. Test via web: http://yourserver/vicidial-did-optimizer.php?test=1\n";
    }
}

// Main execution logic
$optimizer = new DIDOptimizer();
